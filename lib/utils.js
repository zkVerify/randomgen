...
// Maximum 31 bytes (248 bits) to fit in a BN254 field element
const MAX_FIELD_BYTES = 31;

/**
 * Convert various input types to BigInt, truncating to 31 bytes if needed.
 * Accepts: BigInt, number, hex string (0x...), decimal string, Buffer, Uint8Array.
 * 
 * @param {BigInt|string|number|Buffer|Uint8Array} value - Input value
 * @returns {BigInt} Value as BigInt, truncated to 31 bytes
 * @private
 */
function toFieldElement(value) {
  ...
  if (Buffer.isBuffer(value) || value instanceof Uint8Array) {
    // Already bytes - take last 31 bytes if longer
    bytes = value.length > MAX_FIELD_BYTES
      ? value.slice(value.length - MAX_FIELD_BYTES)
      : value;
  ...
  // Truncate to 31 bytes (248 bits) using bitmask
  const MAX_31_BYTES = (1n << 248n) - 1n;
  return bigVal & MAX_31_BYTES;
}
 ...
 * Accepts: BigInt, number, hex string (0x...), decimal string, Buffer, Uint8Array.
 * 
 * @param {BigInt|string|number|Buffer|Uint8Array} blockHash - First input (required)
 * @param {BigInt|string|number|Buffer|Uint8Array} userNonce - Second input (required)
 * @returns {Promise<BigInt>} Poseidon hash result as BigInt
 * 
 * @example
 * const hash = await computePoseidonHash(100, 200);
 * const hash2 = await computePoseidonHash(Buffer.from('...'), 7n);
 */
async function computePoseidonHash(blockHash, userNonce) {
  ...
  // Convert and truncate inputs to 31 bytes
  const inputs = [
    toFieldElement(blockHash),
    toFieldElement(userNonce),
  ];
 ...
 * @param {BigInt|string|number|Buffer|Uint8Array} inputs.blockHash - Public blockchain hash (required)
 * @param {BigInt|string|number|Buffer|Uint8Array} inputs.userNonce - Public user nonce (required)
...
function createCircuitInputs(inputs) {
  ...
  // Convert and truncate to 31 bytes (248 bits) to fit in field element
  const blockHashBig = toFieldElement(blockHash);
  const userNonceBig = toFieldElement(userNonce);
