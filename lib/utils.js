 ...
 * a permutation from the seed. This produces unique shuffled values 1..n.
 * 
 * Note: The circuit uses only the lower 250 bits of the seed.
 * 
 * @param {BigInt} seed - The hash seed for permutation
 * @param {number} n - Size of permutation (items 1 to n, n <= 50)
 * @returns {number[]} Permuted array of values 1 to n
 * @throws {Error} If n > 50
 * 
 * @example
 * const permuted = computePermutation(12345n, 10);
 * // Returns shuffled array like [3, 7, 1, 9, 4, 2, 10, 6, 8, 5]
 */
function computePermutation(seed, n) {
  if (n > 50) {
    throw new Error("n must be <= 50 for RandomPermutate circuit");
  }
  ...
  // Initialize input array [1, 2, ..., n] - matches circuit's in[i] <== i + 1
  const inArr = Array.from({ length: n }, (_, i) => i + 1);
 ...
 * @param {string} circuitName - Circuit name (required): 'random_5_35' etc.
