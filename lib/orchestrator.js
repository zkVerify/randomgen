 ...
 * RandomPermutate to generate shuffled unique numbers from 1 to maxOutputVal.
 * 
 * @example
...
class RandomCircuitOrchestrator {
   ...
   * @param {string} options.circuitName - Circuit name: 'random_3_50', 'random_6_50' etc. (default: 'random_5_35')
   * @param {number} options.numOutputs - Number of random outputs from circuit (default: 5)
   * @param {number} options.maxOutputVal - Maximum value in output range [1, maxOutputVal] (default: 35)
   * @param {number} options.power - Powers of tau size for setup (default: )
  ...
  constructor(options = {}) {
    ...
    this.maxOutputVal = options.maxOutputVal ?? 35;
    this.circuitName = options.circuitName ?? `random_${this.numOutputs}_${this.maxOutputVal}`;
   ...
   * 2. Permutes [1, 2, ..., maxOutputVal] using seed
   * 3. Returns first numOutputs values from permutation
   * 
   * Outputs are unique natural numbers in range [1, maxOutputVal].
   * 
 ...
 * 1. seed = Poseidon(blockHash, userNonce)
 * 2. Permute [1, 2, ..., maxOutputVal] using seed
 ...
 * @param {number} maxOutputVal - Maximum output value (default: 50)
 * @returns {Promise<object>} { seed (BigInt string), randomNumbers (array of numbers) }
 */
async function computeLocalRandomNumbers(inputs, numOutputs, maxOutputVal = 50) {
  if (numOutputs === undefined || numOutputs === null) {
    throw new Error("numOutputs is required");
  }
  if (numOutputs > maxOutputVal) {
    throw new Error("numOutputs must be <= maxOutputVal");
  }

  // Compute Poseidon hash (single output)
  ...
  // Compute permutation
  const permuted = computePermutation(seed, maxOutputVal);
