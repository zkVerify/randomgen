 ...
 * - random_1.circom: numOutputs=15, power=15 (production)
 * - random_3.circom: numOutputs=3, power=13 (testing)
 * 
 * @example
...
class RandomCircuitOrchestrator {
   ...
   * @param {number} options.numOutputs - Number of random outputs from circuit (default: 1)
   * @param {string} options.circuitName - Circuit name (default: 'random_${this.numOutputs}')
  ...
  constructor(options = {}) {
    ...
    this.numOutputs = options.numOutputs ?? 1;
    this.circuitName = options.circuitName ?? `random_${this.numOutputs}`;
    this.power = options.power ?? 11;
   ...
   * 
   * The circuit computes R[i] = PoseidonEx(blockHash, userNonce)[i] mod N
   ...
   * @param {BigInt|string|number} inputs.blockHash - Public blockchain hash (truncated to 31 bytes)
   * @param {BigInt|string|number} inputs.userNonce - Public user nonce (truncated to 31 bytes)
   * @param {BigInt|string|number} inputs.N - Public modulus (R values will be in [0, N), truncated to 31 bytes)
 ...
 * 
 * @param {object} inputs - Circuit inputs
 * @param {BigInt|string|number} inputs.blockHash - Blockchain hash (required)
 * @param {BigInt|string|number} inputs.userNonce - User nonce (required)
 * @param {BigInt|string|number} inputs.N - Modulus for R values (required)
...
async function computeLocalHash(inputs, numOutputs) {
  ...
  if (inputs.blockHash === undefined || inputs.blockHash === null) {
    throw new Error("inputs.blockHash is required");
  }
  if (inputs.userNonce === undefined || inputs.userNonce === null) {
    throw new Error("inputs.userNonce is required");
  }
  if (inputs.N === undefined || inputs.N === null) {
    throw new Error("inputs.N is required");
  }

  const preparedInputs = createCircuitInputs(inputs);
  const NBig = BigInt(preparedInputs.N);

  // Compute Poseidon hash with multiple outputs (always returns array)
  const hashes = await computePoseidonHash(
    BigInt(preparedInputs.blockHash),
    BigInt(preparedInputs.userNonce),
    numOutputs
  );
